...
async function handleLogin() {
  if (!email.value || !password.value) {
    toastStore.error('Por favor completa todos los campos');
    return;
  }
  ...
  try {
    ...
    // Fetch fresh profile data to ensure we have the image
    await authStore.hydrateUser();

    // Fetch initial favorites
    await favStore.fetchFavorites();
    
    toastStore.success(`Bienvenido de nuevo, ${response.user.username}`);
    router.push('/home');
  } catch (err: any) {
    console.error('Login failed:', err);
    if (!err.response) {
       toastStore.error('Error de red. Intenta de nuevo.');
    }
  } finally {
    loading.value = false;
  }
...
<style scoped>
/* Layout: full-screen, centrado, ambient glow */
[data-page="auth"] {
  min-height: 100vh;
  display: grid;
  place-items: center;
...
[data-page="auth"]::before {
  ...
  background:
    radial-gradient(900px circle at 18% 15%, rgba(177, 0, 255, 0.12), transparent 55%),
    radial-gradient(1100px circle at 85% 35%, rgba(124, 0, 255, 0.10), transparent 60%),
    radial-gradient(900px circle at 50% 95%, rgba(177, 0, 255, 0.06), transparent 55%);
}
...
/* Footer link */
[data-card="login"] > div:last-child {
